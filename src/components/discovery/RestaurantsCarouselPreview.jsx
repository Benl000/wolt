...
export const RestaurantsCarouselPreview = ({ restaurant }) => {
  ...
  return (
    <section
      ...
      onMouseDown={(ev) => handleOnMouseDown(ev)}
      onClick={(ev) => handleOnClick(ev, restaurant)}
    >
      <img
        src={restaurant?.listimage}
        alt={restaurant?.name?.[0]?.value || "Restaurant"}
      />
      <div className="restaurant-preview-header">
        <div className="headlines">
          <h3>{restaurant?.name?.[0]?.value || "Unnamed"}</h3>
